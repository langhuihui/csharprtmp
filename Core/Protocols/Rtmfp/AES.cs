...
{
    partial class AESEngine
    {
        #region

        private static readonly UInt32[] Te0 = 
        {
        ...
        private static readonly UInt32[] Te1 =
        {
        ...
        private static readonly UInt32[] Te2 =
        {
        ...
        private static readonly UInt32[] Te3 =
        {
        ...
        private static readonly UInt32[] Td0 =
        {
        ...
        private static readonly UInt32[] Td1 =
        {
        ...
        private static readonly UInt32[] Td2 = 
        {
        ...
        private static readonly UInt32[] Td3 = 
        {
        ...
        private static readonly UInt32[] Td4 = 
        {
        ...
        private static readonly UInt32[] rcon =
        {
        ...
        //private static UInt32 _lrotl(UInt32 x, byte num)
        //{
        // return x << num | x >> (32 - num);
        //}

        //private static UInt32 _lrotr(UInt32 x, byte num)
        //{
        // return x >> num | x << (32 - num);
        //}

        //private int AES_set_decrypt_key(string userKey, int bits)
        //{
        // int i, j, status;
        // UInt32 temp;
        // SetEncryptKey(userKey, bits);


        // for (i = 0, j = 4 * (Encrypkey.rounds); i < j; i += 4, j -= 4)
        // {
        // temp = Encrypkey.rd_key[i]; Encrypkey.rd_key[i] = Encrypkey.rd_key[j]; Encrypkey.rd_key[j] = temp;
        // temp = Encrypkey.rd_key[i + 1]; Encrypkey.rd_key[i + 1] = Encrypkey.rd_key[j + 1]; Encrypkey.rd_key[j + 1] = temp;
        // temp = Encrypkey.rd_key[i + 2]; Encrypkey.rd_key[i + 2] = Encrypkey.rd_key[j + 2]; Encrypkey.rd_key[j + 2] = temp;
        // temp = Encrypkey.rd_key[i + 3]; Encrypkey.rd_key[i + 3] = Encrypkey.rd_key[j + 3]; Encrypkey.rd_key[j + 3] = temp;
        // }

        // for (i = 1; i < (Encrypkey.rounds); i++)
        // {
        // Encrypkey.rd_key[0 + 4 * i] =
        // Td0[Te1[(Encrypkey.rd_key[0 + 4 * i] >> 24)] & 0xff] ^
        // Td1[Te1[(Encrypkey.rd_key[0 + 4 * i] >> 16) & 0xff] & 0xff] ^
        // Td2[Te1[(Encrypkey.rd_key[0 + 4 * i] >> 8) & 0xff] & 0xff] ^
        // Td3[Te1[(Encrypkey.rd_key[0 + 4 * i]) & 0xff] & 0xff];
        // Encrypkey.rd_key[1] =
        // Td0[Te1[(Encrypkey.rd_key[1 + 4 * i] >> 24)] & 0xff] ^
        // Td1[Te1[(Encrypkey.rd_key[1 + 4 * i] >> 16) & 0xff] & 0xff] ^
        // Td2[Te1[(Encrypkey.rd_key[1 + 4 * i] >> 8) & 0xff] & 0xff] ^
        // Td3[Te1[(Encrypkey.rd_key[1 + 4 * i]) & 0xff] & 0xff];
        // Encrypkey.rd_key[2] =
        // Td0[Te1[(Encrypkey.rd_key[2 + 4 * i] >> 24)] & 0xff] ^
        // Td1[Te1[(Encrypkey.rd_key[2 + 4 * i] >> 16) & 0xff] & 0xff] ^
        // Td2[Te1[(Encrypkey.rd_key[2 + 4 * i] >> 8) & 0xff] & 0xff] ^
        // Td3[Te1[(Encrypkey.rd_key[2 + 4 * i]) & 0xff] & 0xff];
        // Encrypkey.rd_key[3] =
        // Td0[Te1[(Encrypkey.rd_key[3 + 4 * i] >> 24)] & 0xff] ^
        // Td1[Te1[(Encrypkey.rd_key[3 + 4 * i] >> 16) & 0xff] & 0xff] ^
        // Td2[Te1[(Encrypkey.rd_key[3 + 4 * i] >> 8) & 0xff] & 0xff] ^
        // Td3[Te1[(Encrypkey.rd_key[3 + 4 * i]) & 0xff] & 0xff];
        // }
        // return 0;
        //}
        ...
        private const int MAXKB = 256/8;
        private const int MAXNR = 14;
        private const int bits = 128;
        private const int rounds = 10;
        //private static byte[] iv; 此处不能设置iv我静态字段, 因为可能多个线程在加解密, 而iv在这些过程中是会改变的.
        //private static string ivStr;
        //public static readonly Stack<byte[]> VIS = new Stack<byte[]>(); 
        private readonly byte[] _iv = new byte[AES_BLOCK_SIZE];
        ...
        private static UInt32 GetU32(byte[] str,int offset = 0) //==? 要使用上面的还是这个
        {
            return (UInt32)str[offset] << 24 ^ (UInt32)str[1 + offset] << 16 ^ (UInt32)str[2 + offset] << 8 ^ (UInt32)str[3 + offset];
        }
        ...
        private static void U32ToByteArray(UInt32 s0, UInt32 s1, UInt32 s2, UInt32 s3, byte[] byteArray, int start)
        {
            //if (byteArray.Length == AES_BLOCK_SIZE)
            //    start = 0;
            byteArray[0 + start] = (byte) (s0 >> 24);
            byteArray[1 + start] = (byte) (s0 >> 16); //?
            byteArray[2 + start] = (byte) (s0 >> 8); //?
            byteArray[3 + start] = (byte) (s0); //?

            byteArray[4 + start] = (byte) (s1 >> 24);
            byteArray[5 + start] = (byte) (s1 >> 16); //?
            byteArray[6 + start] = (byte) (s1 >> 8); //?
            byteArray[7 + start] = (byte) (s1); //?

            byteArray[8 + start] = (byte) (s2 >> 24);
            byteArray[9 + start] = (byte) (s2 >> 16); //?
            byteArray[10 + start] = (byte) (s2 >> 8); //?
            byteArray[11 + start] = (byte) (s2); //?

            byteArray[12 + start] = (byte) (s3 >> 24);
            byteArray[13 + start] = (byte) (s3 >> 16); //?
            byteArray[14 + start] = (byte) (s3 >> 8); //?
            byteArray[15 + start] = (byte) (s3); //?
        }
        ...
        /// <returns></returns>
        private void Encrypt16Byte(byte[] outArray, int start, UInt32[] key)
        {
            UInt32 s0, s1, s2, s3, t0, t1, t2, t3;
            ByteArrayToU32(outArray, start, out s0, out s1, out s2, out s3);
            s0 = s0 ^ key[0];
            s1 = s1 ^ key[1];
            s2 = s2 ^ key[2];
            s3 = s3 ^ key[3];
            ...
            U32ToByteArray(s0, s1, s2, s3, outArray, start);
        }
        ...
        //public static UInt32 Compute(UInt32[] ary)
        //{
        //    UInt32 myTmep = 0;
